...
- **Pomegranate-derived definitions**: 313 definitions converted from the
  ...
  catalog. The large Pomegranate portal long-form audit table is not required
  for endpoint construction and is not shipped in the package build.
...
catalog pinned in the source manifest. If users keep a local portal audit CSV,
they can load it explicitly with `load_pomegranate_portal_coding(path = ...)`;
this optional audit table is not needed for ordinary endpoint construction.
