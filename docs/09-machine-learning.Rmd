...
This chapter focuses on binary classification, multiclass classification, and
continuous outcomes. Time-to-event machine learning remains available through
`ukb_ml_survival()` and is described separately at the end.
...
The frozen-test workflow above is for non-survival outcomes. For time-to-event
prediction, use the existing survival ML interface.
...
install.packages(c(
  ...
  "randomForestSRC"  # survival random forest
))
```
...
needed if you already installed it for SVM.
