...
ukb_demo <- function(n = NULL, seed = 20260618L) {
  ...
  generate_demo <- function() {
    ...
    education <- sample(c(1L, 2L, 3L, 4L, 5L, 6L, -3L), n,
      replace = TRUE,
      prob = c(0.28, 0.18, 0.25, 0.08, 0.11, 0.08, 0.02)
    )
    smoking <- sample(c(0L, 1L, 2L, -3L), n,
      replace = TRUE,
      prob = c(0.55, 0.32, 0.11, 0.02)
    )
    drinking <- sample(c(0L, 1L, 2L, -3L), n,
      replace = TRUE,
    ...
    if (length(sr_code_cols) > 0L) {
      ...
      out[[sr_code_cols[[1L]]]] <- sample(c(sr_codes, NA_integer_), n,
        replace = TRUE,
    ...
    missing_cols <- intersect(
      c(
        grep("^p21001_", cols, value = TRUE), grep("^p20116_", cols, value = TRUE),
